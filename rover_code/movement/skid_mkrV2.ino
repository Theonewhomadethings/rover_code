...
// version 2 
// - attempts to fix the centring issue with rc joystick being higher than expected
// - also attempts to fix the forward joystick movement being inverted
// - known bug: when joystick is moved forward slightly the movement is full speed forward, when joystick is moved forward fully to top position, the movement is slow forward. We expect this behaviour to be inversed. Weirdly the behavior for backwards, left and right are all nominal. So unclear if this is a hardware or software bug.
