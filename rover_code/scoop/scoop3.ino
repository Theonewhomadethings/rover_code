...
const int MIN_VELOCITY = -200;  // Define minimum servo velocity
const int MAX_VELOCITY = 200;   // Define maximum servo velocity
const int DEADZONE = 100;       // Define deadband range

void setup() {
  // Initialize serial
  Serial.begin(9600);
  delay(100);   // Keep to prevent breaking the arms
...
void loop() {
  ...
  int leftArmVelocity = 0, rightArmVelocity = 0, scoopVelocity = 0;

  // Check for change in throttle pulse
  if (throttlePulse != previousThrottlePulse) {
    previousThrottlePulse = throttlePulse;

    // Map throttlePulse (1000 to 2000) to velocity (-200 to 200)
    int throttleVelocity = map(throttlePulse, 1000, 2000, MIN_VELOCITY, MAX_VELOCITY);

    // Apply deadband
    if (throttlePulse >= 1500 - DEADZONE && throttlePulse <= 1500 + DEADZONE) {
      // Within deadband, stop the servos
      leftArmVelocity = 0;
      rightArmVelocity = 0;
    } else {
  ...
  if (rudderPulse != previousRudderPulse) {
    ...
    // Map rudderPulse (1000 to 2000) to velocity (-200 to 200)
    scoopVelocity = map(rudderPulse, 1000, 2000, MIN_VELOCITY, MAX_VELOCITY);

    // Apply deadband
    if (rudderPulse >= 1500 - DEADZONE && rudderPulse <= 1500 + DEADZONE) {
      scoopVelocity = 0;
    }
