...
void setup() {
  ...
  initializeServo(DXL_ID_ARMS, OP_POSITION); // Position control for arm servos
  initializeServo(DXL_ID_SCOOP, OP_VELOCITY); // Velocity control for scoop tilt servo
...
void loop() {
  ...
  int scoopVelocity = rudderControl * 1023; // Scale to full velocity range

  // Set the scoop tilt servo velocity
  dxl.setGoalVelocity(DXL_ID_SCOOP, scoopVelocity);
